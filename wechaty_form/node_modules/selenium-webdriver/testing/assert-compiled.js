  ...
  instanceOf(ctor, opt_message) {
    ...
    return evaluate(this.subject_, function (actual) {
      if (!(actual instanceof ctor)) {
        assert.fail(opt_message || `${ describe(actual) } instanceof ${ ctor.name || ctor }`);
      }
    });
  }
  ...
  contains(value, opt_message) {
    return evaluate(this.subject_, function (actual) {
      if (actual instanceof Map || actual instanceof Set) {
        ...
        assert.ok(actual.indexOf(value) !== -1, opt_message || `${ actual }.indexOf(${ value }) !== -1`);
      } else {
        assert.fail(`Expected an array, map, set, or string: got ${ describe(actual) }`);
      }
    });
  }
  ...
  matches(regex, opt_message) {
    ...
    return evaluate(this.subject_, function (actual) {
      if (!isString(actual) || !regex.test(actual)) {
        let message = opt_message || `Expected a string matching ${ regex }, got ${ describe(actual) }`;
