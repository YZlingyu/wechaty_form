...
function resposition(driver) {
  return driver.manage().window().setSize(600, 400).then(_ => driver.manage().window().setPosition(300 * i++, 0));
}
...
function doSearch(driver) {
  ...
  // Now do the rest.
  .then(_ => driver.get('http://www.google.com/ncr')).then(_ => driver.findElement(By.name('q')).sendKeys('webdriver')).then(_ => driver.findElement(By.name('btnG')).click()).then(_ => driver.wait(until.titleIs('webdriver - Google Search'), 1000)).then(_ => driver.quit());
}
...
Promise.all([doSearch(createDriver(Channel.RELEASE)), doSearch(createDriver(Channel.AURORA)), // Developer Edition.
doSearch(createDriver(Channel.NIGHTLY))]).then(_ => {
  console.log('Success!');
}, err => {
...
//# sourceMappingURL=firefox_channels-compiled.js.map