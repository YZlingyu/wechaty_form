...
app.get('/', sendIndex).get('/favicon.ico', function (req, res) {
  res.writeHead(204);
  res.end();
...
function sendInifinitePage(request, response) {
  ...
  var body = ['<!DOCTYPE html>', '<title>Page', pageNumber, '</title>', 'Page number <span id="pageNumber">', pageNumber, '</span>', '<p><a href="../xhtmlTest.html" target="_top">top</a>'].join('');
...
function sendEcho(request, response) {
  var body = ['<!DOCTYPE html>', '<title>Echo</title>', '<div class="request">', request.method, ' ', request.url, ' ', 'HTTP/', request.httpVersion, '</div>'];
...
function sendIndex(request, response) {
  ...
  var data = ['<!DOCTYPE html><h1>/</h1><hr/><ul>', createListEntry('common')];
  if (isDevMode) {
...
//# sourceMappingURL=fileserver-compiled.js.map