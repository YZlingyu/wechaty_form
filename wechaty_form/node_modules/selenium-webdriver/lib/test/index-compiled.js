 ...
 */
var NATIVE_BROWSERS = [webdriver.Browser.CHROME, webdriver.Browser.EDGE, webdriver.Browser.FIREFOX, LEGACY_FIREFOX, webdriver.Browser.IE, webdriver.Browser.OPERA, webdriver.Browser.PHANTOM_JS, webdriver.Browser.SAFARI];
...
var browsersToTest = function () {
  ...
  browsers.forEach(function (browser) {
    ...
    if (NATIVE_BROWSERS.indexOf(parts[0]) == -1 && !permitRemoteBrowsers) {
      throw Error('Browser ' + parts[0] + ' requires a WebDriver server and ' + 'neither the SELENIUM_REMOTE_URL nor the SELENIUM_SERVER_JAR ' + 'environment variables have been set.');
    }

    var recognized = false;
    for (var prop in webdriver.Browser) {
      if (webdriver.Browser.hasOwnProperty(prop) && webdriver.Browser[prop] === parts[0]) {
        recognized = true;
        break;
      }
    }
  ...
  console.log('Promise manager is enabled? ' + webdriver.promise.USE_PROMISE_MANAGER);
  ...
  try {
    ...
    before(function () {
      if (isDevMode && !noBuild) {
        return build.of('//javascript/atoms/fragments:is-displayed', '//javascript/webdriver/atoms:getAttribute').onlyOnce().go();
      }
    });
      ...
      describe('[' + browser + ']', function () {
        ...
        if (!!serverJar && !remoteUrl) {
          if (!(serverToUse = seleniumServer)) {
            serverToUse = seleniumServer = new remote.SeleniumServer(serverJar, { loopback: useLoopback });
          }
