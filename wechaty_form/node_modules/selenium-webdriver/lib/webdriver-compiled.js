...
class WebDriver {
   ...
   */
  static createSession(executor, capabilities, opt_flow, opt_ctor, opt_onQuit) {
    let flow = opt_flow || promise.controlFlow();
  ...
  /** @override */
  quit() {
    var result = this.schedule(new command.Command(command.Name.QUIT), 'WebDriver.quit()');
  ...
  wait(condition, opt_timeout, opt_message) {
    if (promise.isPromise(condition)) {
      ...
      /** @type {!IThenable} */condition, opt_timeout, opt_message);
    }
...
class Options {
  ...
  addCookie(spec) {
    ...
    let cookieString = name + '=' + value + (spec.domain ? ';domain=' + spec.domain : '') + (spec.path ? ';path=' + spec.path : '') + (spec.secure ? ';secure' : '');
...
class TargetLocator {
   ...
   */
  window(nameOrHandle) {
    return this.driver_.schedule(new command.Command(command.Name.SWITCH_TO_WINDOW).
...
class WebElement {
  ...
  static buildId(id, opt_noLegacy) {
    return opt_noLegacy ? { [ELEMENT_ID_KEY]: id } : { [ELEMENT_ID_KEY]: id, [LEGACY_ELEMENT_ID_KEY]: id };
  }
  ...
  sendKeys(var_args) {
    ...
    if (!this.driver_.fileDetector_) {
      return this.schedule_(new command.Command(command.Name.SEND_KEYS_TO_ELEMENT).setParameter('text', keys).setParameter('value', keys), 'WebElement.sendKeys()');
    }
...
//# sourceMappingURL=webdriver-compiled.js.map