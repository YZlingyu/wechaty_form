...
test.suite(function (env) {
  ...
  describe('finding elements', function () {
    test.it('should work after loading multiple pages in a row', function* () {
    ...
    describe('By.id()', function () {
      ...
      test.it('should fail if ID not present on page', function* () {
        yield driver.get(Pages.formPage);
        return driver.findElement(By.id('nonExistantButton')).then(fail, function (e) {
          assert(e).instanceOf(error.NoSuchElementError);
        });
      });

      test.it('should find multiple elements by ID even though that is ' + 'malformed HTML', function* () {
    ...
    describe('By.linkText()', function () {
      ...
      test.it('should be able to find elements by partial link text', function* () {
        yield driver.get(Pages.xhtmlTestPage);
      ...
      test.it('works when searching for multiple and text contains =', function* () {
        yield driver.get(Pages.xhtmlTestPage);
        let elements = yield driver.findElements(By.linkText('Link=equalssign'));
      ...
      test.it('works when searching for multiple with partial text containing =', function* () {
        yield driver.get(Pages.xhtmlTestPage);
        let elements = yield driver.findElements(By.partialLinkText('Link='));
      ...
      test.it('should be able to find multiple exact matches', function* () {
        yield driver.get(Pages.xhtmlTestPage);
        let elements = yield driver.findElements(By.linkText('click me'));
        assert(elements.length).equalTo(2);
      });

      test.it('should be able to find multiple partial matches', function* () {
        yield driver.get(Pages.xhtmlTestPage);
        let elements = yield driver.findElements(By.partialLinkText('ick me'));
        assert(elements.length).equalTo(2);
      });

      test.ignore(browsers(Browser.SAFARI)).it('works on XHTML pages', function* () {
    ...
    describe('By.name()', function () {
      ...
      test.it('should be able to find elements that do not support name property', function* () {
        yield driver.get(Pages.nestedPage);
    ...
    describe('By.className()', function () {
      ...
      test.it('should fail if queried name only partially matches', function* () {
        yield driver.get(Pages.xhtmlTestPage);
        return driver.findElement(By.className('nameB')).then(fail, function (e) {
          assert(e).instanceOf(error.NoSuchElementError);
        });
      });
      ...
      test.it('permits compound class names', function () {
        return driver.get(Pages.xhtmlTestPage).then(() => driver.findElement(By.className('nameA nameC'))).then(el => el.getText()).then(text => assert(text).equalTo('An H2 title'));
      });
    });
    ...
    describe('By.css()', function () {
      ...
      test.it('should find first matching element when searching by ' + 'compound CSS selector', function* () {
        yield driver.get(Pages.xhtmlTestPage);

        let el = yield driver.findElement(By.css('div.extraDiv, div.content'));
        yield assert(el.getAttribute('class')).equalTo('content');
      });

      test.it('should be able to find multiple elements by compound selector', function* () {
        yield driver.get(Pages.xhtmlTestPage);
        let elements = yield driver.findElements(By.css('div.extraDiv, div.content'));
      ...
      test.ignore(browsers(Browser.IE)).it('should be able to find element by boolean attribute', function* () {
        yield driver.get(test.whereIs('locators_tests/boolean_attribute_selected.html'));

        let el = yield driver.findElement(By.css('option[selected="selected"]'));
        yield assert(el.getAttribute('value')).equalTo('two');
      });

      test.it('should be able to find element with short ' + 'boolean attribute selector', function* () {
        yield driver.get(test.whereIs('locators_tests/boolean_attribute_selected.html'));
      ...
      test.it('should be able to find element with short boolean attribute ' + 'selector on HTML4 page', function* () {
        yield driver.get(test.whereIs('locators_tests/boolean_attribute_selected_html4.html'));
    ...
    describe('by custom locator', function () {
      ...
      test.it('fails if locator returns non-webelement value', function* () {
        ...
        return link.then(() => fail('Should have failed'), e => assert(e).instanceOf(TypeError));
      });
    });
    ...
    describe('switchTo().activeElement()', function () {
      ...
      test.ignore(browsers(Browser.SAFARI)).it('returns document.activeElement', function* () {
        yield driver.get(Pages.formPage);
        ...
        let equal = yield driver.executeScript('return arguments[0] === arguments[1]', email, ae);
        assert(equal).isTrue();
      });
    });
  });
...
//# sourceMappingURL=element_finding_test-compiled.js.map