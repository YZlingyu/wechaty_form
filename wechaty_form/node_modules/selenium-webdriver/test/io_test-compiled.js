...
describe('io', function () {
      ...
      return io.tmpDir().then(function (dir) {
        ...
        return io.tmpDir().then(function (dst) {
          return io.copyDir(dir, dst).then(function (ret) {
            ...
            assert.equal('goodbye', fs.readFileSync(path.join(dst, 'sub/folder/file2')));
          });
        });
      });
    });
  ...
  describe('exists', function () {
    ...
    it('returns a rejected promise if input value is invalid', function () {
      return io.exists(undefined).then(() => assert.fail('should have failed'), e => assert.ok(e instanceof TypeError));
    });
  ...
  describe('read', function () {
    ...
    it('catches errors from invalid input', function () {
      return io.read({}).then(() => assert.fail('should have failed'), e => assert.ok(e instanceof TypeError));
    });

    it('rejects returned promise if file does not exist', function () {
      return io.read(path.join(tmpDir, 'not-there')).then(() => assert.fail('should have failed'), e => assert.equal('ENOENT', e.code));
    });
  });
});

//# sourceMappingURL=io_test-compiled.js.map