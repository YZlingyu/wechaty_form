...
describe('promise', function () {
  ...
  afterEach(function tearDown() {
    if (promise.USE_PROMISE_MANAGER) {
      ...
      assert.deepEqual([], uncaughtExceptions, 'Did not expect any uncaught exceptions');
      promise.LONG_STACK_TRACES = false;
    }
  });
  ...
  enablePromiseManager(() => {
    ...
    describe('then', function () {
      ...
      it('stillConsideredUnHandledIfNoCallbacksWereGivenOnCallsToThen', function () {
        ...
        .then(() => new NativePromise(resolve => setTimeout(resolve, 0))).then(() => handler.assertCalled());
      });
    });
    ...
    describe('finally', function () {
      it('nonFailingCallbackDoesNotSuppressOriginalError', function () {
        ...
        return promise.rejected(new StubError()).finally(function () {}).catch(done).finally(done.assertCalled);
      });

      it('failingCallbackSuppressesOriginalError', function () {
        var done = callbackHelper(assertIsStubError);
        return promise.rejected(new Error('original')).finally(throwStubError).catch(done).finally(done.assertCalled);
      });

      it('callbackThrowsAfterFulfilledPromise', function () {
        var done = callbackHelper(assertIsStubError);
        return promise.fulfilled().finally(throwStubError).catch(done).finally(done.assertCalled);
      });

      it('callbackReturnsRejectedPromise', function () {
        var done = callbackHelper(assertIsStubError);
        return promise.fulfilled().finally(function () {
          return promise.rejected(new StubError());
        }).catch(done).finally(done.assertCalled);
      });
    });
    ...
    describe('cancel', function () {
      ...
      describe('can cancel original promise from its child;', function () {
        ...
        it('child linked by resolving with parent', function () {
          ...
          return parent.promise.then(() => assert.fail('expected a cancellation'), e => {
            assert.ok(e instanceof promise.CancellationError);
        ...
        it('grand child through thenable chain', function () {
          ...
          return p.then(() => assert.fail('expected to be cancelled'), e => {
            assert.ok(e instanceof promise.CancellationError);
        ...
        it('"parent" is a CancellableThenable', function () {
          ...
          function assertStopped(p) {
            return p.then(() => assert.fail('not stopped!'), e => {
              assert.ok(e instanceof promise.CancellationError);
              assert.equal('stop!', e.message);
            });
          }
      ...
      it('canCancelATimeout', function () {
        var p = promise.delayed(25).then(assert.fail, e => e instanceof promise.CancellationError);
  ...
  promiseManagerSuite(() => {
    ...
    describe('fullyResolved', function () {
      it('primitives', function () {
        function runTest(value) {
          ...
          return promise.fullyResolved(value).then(resolved => assert.equal(value, resolved));
        }
        return runTest(true).then(() => runTest(function () {})).then(() => runTest(null)).then(() => runTest(123)).then(() => runTest('foo bar')).then(() => runTest(undefined));
      });
      ...
      it('promiseResolvesToPrimitive', function () {
        return promise.fullyResolved(Promise.resolve(123)).then(resolved => assert.equal(123, resolved));
      });
      ...
      it('rejectsOnFirstArrayRejection', function () {
        ...
        var aPromise = Promise.resolve([createRejectedPromise(e1), createRejectedPromise(e2)]);
      ...
      it('rejectsIfNestedArrayPromiseRejects', function () {
        var aPromise = Promise.resolve([Promise.resolve([createRejectedPromise(new StubError())])]);
    ...
    describe('all', function () {
      it('(base case)', function () {
        ...
        var a = [0, 1, deferredObjs[0].promise, deferredObjs[1].promise, 4, 5, 6];
        delete a[5];
    ...
    describe('map', function () {
      ...
      it('rejectsWithFirstRejectedPromise', function () {
        var innerResult = [Promise.resolve(), createRejectedPromise(new StubError()), createRejectedPromise(Error('should be ignored'))];
        var count = 0;
    ...
    describe('testLongStackTraces', function () {
      ...
      it('appendsInitialPromiseCreation_resolverThrows', function () {
        ...
        return new promise.Promise(function () {
          ...
          assert.equal(e.stack.indexOf(originalStack), 0, 'should start with original stack');
      ...
      it('appendsEachStepToRejectionError', function () {
        ...
        return new promise.Promise(function () {
          ...
          assert.notEqual(originalStack, e.stack);
          assert.equal(e.stack.indexOf(originalStack), 0, 'should start with original stack');
          assert.deepEqual(['From: ManagedPromise: new', 'From: Promise: then', 'From: Promise: catch', 'From: Promise: then', 'From: Promise: catch'], getStackMessages(e));
        });
      });
      ...
      it('errorOccursInCallbackChain', function () {
        ...
        return promise.fulfilled().then(function () {}).then(function () {}).then(function () {
          try {
            throw error;
          } catch (e) {
            originalStack = e.stack;
          ...
          assert.notEqual(originalStack, e.stack);
          assert.equal(e.stack.indexOf(originalStack), 0, 'should start with original stack');
          assert.deepEqual(['From: Promise: then', 'From: Promise: catch'], getStackMessages(e));
        });
      });
    });
  });
...
//# sourceMappingURL=promise_test-compiled.js.map