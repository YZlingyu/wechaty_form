  ...
  enablePromiseManager(() => {
    ...
    describe('multiple unhandled rejections outside a task', function () {
      it('are reported in order they occurred', function () {
        ...
        return waitForAbort(flow).then(function (error) {
          assert.ok(error instanceof promise.MultipleUnhandledRejectionError);
    ...
    it('testTaskHasUnhandledRejection_cancelsRemainingSubTasks', function () {
      ...
      flow.execute(function () {
        ...
        flow.execute(() => seen.push('a')).then(() => seen.push('b'), e => seen.push(e));
        flow.execute(() => seen.push('c')).then(() => seen.push('b'), e => seen.push(e));
      });

      return waitForAbort().then(assertIsStubError).then(() => assert.deepEqual([], seen));
    });
    ...
    describe('long stack traces', function () {
      ...
      it('always includes task stacks in failures', function () {
        ...
        flow.execute(function () {
          ...
          assert.deepEqual(['From: Task: throw error', 'From: Task: two', 'From: Task: three'], messages);
        });
        return waitForIdle();
      });
      ...
      it('does not include promise chain when disabled', function () {
        ...
        flow.execute(function () {
          flow.execute(function () {
            return promise.fulfilled().then(function () {}).then(function () {}).then(throwStubError);
          ...
          assert.deepEqual(['From: Task: eventually assert.fails', 'From: Task: start'], messages);
        });
        return waitForIdle();
      });
      ...
      it('includes promise chain when enabled', function () {
        ...
        flow.execute(function () {
          ...
          assert.deepEqual(['From: Promise: then', 'From: Task: eventually assert.fails', 'From: Task: start'], messages);
        });
        return waitForIdle();
      });
    });
    ...
    describe('frame cancels remaining tasks', function () {
      ...
      describe('task callbacks scheduled in another frame', function () {
        ...
        function assertCancellation(e) {
          ...
          assert.equal('Task was discarded due to a previous failure: stub', e.message);
        }
        ...
        it('are rejected with cancellation error', function () {
          ...
          }).then(() => result).then(assert.fail, assertCancellation);
        });
        ...
        it('cancellation errors propagate through callbacks (1)', function () {
          ...
          return Promise.resolve().then(function () {
            return flow.execute(function () {
              result = subTask.then(assert.fail, assertCancellation).then(() => 'abc123');
            });
          }).then(() => result).then(value => assert.equal('abc123', value));
        });
        ...
        it('cancellation errors propagate through callbacks (2)', function () {
          ...
          return Promise.resolve().then(function () {
            return flow.execute(function () {
              result = subTask.then(assert.fail).then(noop, assertCancellation).then(() => 'fin');
            });
          })
          // Verify result actually computed successfully all the way through.
          .then(() => result).then(value => assert.equal('fin', value));
        });
      });
    });
    ...
    describe('task callbacks get cancellation error if registered after task was cancelled', function () {
      ...
      it('(b)', function () {
        ...
        flow.execute(function () {
          ...
          task.then(() => seen.push(1)).then(() => seen.push(2));
          task.then(() => seen.push(3)).then(() => seen.push(4));
        }).then(assert.fail, assertIsStubError);
    ...
    it('unhandledRejectionInParallelTaskQueue', function () {
      ...
      return waitForAbort().then(function () {
        assert.deepEqual(['a.1', 'b.1', 'c.1', 'b.2', 'c.2'], seen);
      }).then(() => assertWasCancelled(b3)).then(() => assertWasCancelled(c3));
    });
...
//# sourceMappingURL=promise_error_test-compiled.js.map