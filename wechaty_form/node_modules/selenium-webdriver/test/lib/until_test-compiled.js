...
describe('until', function () {
  ...
  describe('ableToSwitchToFrame', function () {
    ...
    function onSwitchFrame(expectedId) {
      ...
      return cmd => {
        assert.deepEqual(cmd.getParameter('id'), expectedId, 'frame ID not specified');
        return true;
      };
    }
    ...
    it('byLocator_elementNotInitiallyFound', function () {
      ...
      return driver.wait(until.ableToSwitchToFrame(By.id('foo')), 2000).then(() => assert.deepEqual(foundResponses, []));
    });
  ...
  it('testUntilElementsLocated', function () {
    ...
    return driver.wait(until.elementsLocated(By.id('quux')), 2000).then(function (els) {
  ...
  it('testUntilStalenessOf', function () {
    ...
    return driver.wait(until.stalenessOf(el), 2000).then(() => assert.equal(count, 3));
  });
  ...
  describe('element state conditions', function () {
    ...
    it('elementTextIs', function () {
      return runElementStateTest(until.elementTextIs, 'foobar', CommandName.GET_ELEMENT_TEXT, ['foo', 'fooba', 'foobar']);
    });

    it('elementTextContains', function () {
      return runElementStateTest(until.elementTextContains, 'bar', CommandName.GET_ELEMENT_TEXT, ['foo', 'foobaz', 'foobarbaz']);
    });

    it('elementTextMatches', function () {
      return runElementStateTest(until.elementTextMatches, /fo+bar{3}/, CommandName.GET_ELEMENT_TEXT, ['foo', 'foobar', 'fooobarrr']);
    });
  });
  ...
  describe('WebElementCondition', function () {
    it('fails if wait completes with a non-WebElement value', function () {
      ...
      return result.then(() => assert.fail('expected to fail'), function (e) {
        assert.ok(e instanceof TypeError);
        assert.equal('WebElementCondition did not resolve to a WebElement: ' + '[object Number]', e.message);
      });
    });
  });
});

//# sourceMappingURL=until_test-compiled.js.map