...
describe('WebDriver', function () {
  ...
  function waitForAbort(opt_flow, opt_n) {
    var n = opt_n || 1;
    var theFlow = opt_flow || flow;
    theFlow.removeAllListeners(promise.ControlFlow.EventType.UNCAUGHT_EXCEPTION);
    return new Promise(function (fulfill, reject) {
  ...
  describe('testAttachToSession', function () {
    ...
    it('failsToGetSessionInfo', function () {
      ...
      let driver = WebDriver.attachToSession(executor, SESSION_ID);
      return driver.getSession().then(() => assert.fail('should have failed!'), actual => assert.strictEqual(actual, e));
    });
  ...
  describe('testCreateSession', function () {
    ...
    it('invokes quit callback if it fails to create a session', function () {
      ...
      var driver = WebDriver.createSession(executor, { 'browserName': 'firefox' }, null, null, () => called = true);
    ...
    enablePromiseManager(() => {
      ...
      describe('creation failures bubble up in control flow', function () {
        function runTest(...args) {
          ...
          WebDriver.createSession(executor, { 'browserName': 'firefox' }, ...args);
        ...
        it('onQuit callback failure suppress creation failure', function () {
          ...
          WebDriver.createSession(executor, { 'browserName': 'firefox' }, null, null, () => {
            throw e;
          });
  ...
  it('testDoesNotExecuteCommandIfSessionDoesNotResolve', function () {
    ...
    return new FakeExecutor().createDriver(session).getTitle().then(_ => assert.fail('should have failed'), assertIsStubError);
  });
  ...
  it('testCommandReturnValuesArePassedToFirstCallback', function () {
    ...
    return driver.getTitle().then(title => assert.equal('Google Search', title));
  });

  it('testStopsCommandExecutionWhenAnErrorOccurs', function () {
    let e = new error.NoSuchWindowError('window not found');
    let executor = new FakeExecutor().expect(CName.SWITCH_TO_WINDOW).withParameters({
      'name': 'foo',
      'handle': 'foo'
    }).andReturnError(e).end();

    let driver = executor.createDriver();
    return driver.switchTo().window('foo').then(_ => driver.getTitle(), // mock should blow if this gets executed
  ...
  it('testCanSuppressCommandFailures', function () {
    ...
    let executor = new FakeExecutor().expect(CName.SWITCH_TO_WINDOW).withParameters({
      'name': 'foo',
      'handle': 'foo'
    }).andReturnError(e).expect(CName.GET_TITLE).andReturnSuccess('Google Search').end();

    var driver = executor.createDriver();
    driver.switchTo().window('foo').catch(v => assert.strictEqual(v, e));
    driver.getTitle();
    return waitForIdle();
  });
  ...
  it('testErrorsPropagateUpToTheRunningApplication', function () {
    ...
    return executor.createDriver().switchTo().window('foo').then(_ => assert.fail(), v => assert.strictEqual(v, e));
  });
  ...
  it('testErrbacksThatReturnErrorsStillSwitchToCallbackChain', function () {
    let executor = new FakeExecutor().expect(CName.SWITCH_TO_WINDOW).withParameters({
      'name': 'foo',
      'handle': 'foo'
    }).andReturnError(new error.NoSuchWindowError('window not found')).end();
  ...
  it('testErrbacksThrownCanOverrideOriginalError', function () {
    let executor = new FakeExecutor().expect(CName.SWITCH_TO_WINDOW, {
      'name': 'foo',
      'handle': 'foo'
    }).andReturnError(new error.NoSuchWindowError('window not found')).end();

    var driver = executor.createDriver();
    return driver.switchTo().window('foo').catch(throwStubError).then(assert.fail, assertIsStubError);
  });

  it('testReportsErrorWhenExecutingCommandsAfterExecutingAQuit', function () {
    let executor = new FakeExecutor().expect(CName.QUIT).end();

    let verifyError = expectedError(error.NoSuchSessionError, 'This driver instance does not have a valid session ID ' + '(did you call WebDriver.quit()?) and may no longer be used.');

    let driver = executor.createDriver();
    return driver.quit().then(_ => driver.get('http://www.google.com')).then(assert.fail, verifyError);
  });
  ...
  describe('returningAPromise', function () {
    it('fromACallback', function () {
      ...
      var driver = executor.createDriver();
      return driver.getTitle().then(function () {
    ...
    it('fromAnErrbackSuppressesTheError', function () {
      let executor = new FakeExecutor().expect(CName.SWITCH_TO_WINDOW, {
        'name': 'foo',
        'handle': 'foo'
      }).andReturnError(new StubError()).expect(CName.GET_CURRENT_URL).andReturnSuccess('http://www.google.com').end();

      var driver = executor.createDriver();
      return driver.switchTo().window('foo').catch(function (e) {
        assertIsStubError(e);
        return driver.getCurrentUrl();
  ...
  describe('WebElementPromise', function () {
    ...
    it('resolvesBeforeCallbacksOnWireValueTrigger', function () {
      ...
      let steps = [element.then(_ => messages.push('element resolved')), element.getId().then(_ => messages.push('wire value resolved'))];
  ...
  describe('executeScript', function () {
    it('nullReturnValue', function () {
      ...
      var driver = executor.createDriver();
      return driver.executeScript('return document.body;').then(result => assert.equal(null, result));
    });
    ...
    it('primitiveReturnValue', function () {
      ...
      var driver = executor.createDriver();
      return driver.executeScript('return document.body;').then(result => assert.equal(123, result));
    });
    ...
    it('webElementReturnValue', function () {
      ...
      var driver = executor.createDriver();
      return driver.executeScript('return document.body;').then(element => element.getId()).then(id => assert.equal(id, 'foo'));
    });
    ...
    it('arrayReturnValue', function () {
      ...
      var driver = executor.createDriver();
      return driver.executeScript('return document.body;').then(function (array) {
    ...
    it('objectReturnValue', function () {
      ...
      var driver = executor.createDriver();
      var callback;
      return driver.executeScript('return document.body;').then(obj => obj['foo'].getId()).then(id => assert.equal(id, 'foo'));
    });
    ...
    it('webElementPromiseArgumentConversion', function () {
      ...
      let executor = new FakeExecutor().expect(CName.FIND_ELEMENT, { 'using': 'css selector', 'value': '*[id="foo"]' }).andReturnSuccess(elementJson).expect(CName.EXECUTE_SCRIPT).withParameters({
        'script': 'return 1;',
        'args': [elementJson]
      }).andReturnSuccess(null).end();
  ...
  describe('findElement', function () {
    it('elementNotFound', function () {
      let executor = new FakeExecutor().expect(CName.FIND_ELEMENT, { using: 'css selector', value: '*[id="foo"]' }).andReturnError(new StubError()).end();
    ...
    it('elementNotFoundInACallback', function () {
      ...
      var driver = executor.createDriver();
      return Promise.resolve().then(_ => driver.findElement(By.id('foo'))).then(assert.fail, assertIsStubError);
    });

    it('elementFound', function () {
      let executor = new FakeExecutor().expect(CName.FIND_ELEMENT, { using: 'css selector', value: '*[id="foo"]' }).andReturnSuccess(WebElement.buildId('bar')).expect(CName.CLICK_ELEMENT, { 'id': WebElement.buildId('bar') }).andReturnSuccess().end();
    ...
    it('byJs', function () {
      let executor = new FakeExecutor().expect(CName.EXECUTE_SCRIPT, {
        'script': 'return document.body',
        'args': []
      }).andReturnSuccess(WebElement.buildId('bar')).expect(CName.CLICK_ELEMENT, { 'id': WebElement.buildId('bar') }).end();
    ...
    it('byJs_returnsNonWebElementValue', function () {
      let executor = new FakeExecutor().expect(CName.EXECUTE_SCRIPT, { 'script': 'return 123', 'args': [] }).andReturnSuccess(123).end();
    ...
    it('customLocator', function () {
      let executor = new FakeExecutor().expect(CName.FIND_ELEMENTS, { 'using': 'css selector', 'value': 'a' }).andReturnSuccess([WebElement.buildId('foo'), WebElement.buildId('bar')]).expect(CName.CLICK_ELEMENT, { 'id': WebElement.buildId('foo') }).andReturnSuccess().end();
    ...
    it('customLocatorThrowsIfresultIsNotAWebElement', function () {
      var driver = new FakeExecutor().createDriver();
      return driver.findElement(_ => 1).then(assert.fail, function (e) {
        assertIsInstance(TypeError, e);
        assert.equal('Custom locator did not return a WebElement', e.message);
      });
    });
  ...
  describe('findElements', function () {
    it('returnsMultipleElements', function () {
      var ids = ['foo', 'bar', 'baz'];
      let executor = new FakeExecutor().expect(CName.FIND_ELEMENTS, { 'using': 'css selector', 'value': 'a' }).andReturnSuccess(ids.map(WebElement.buildId)).end();

      var driver = executor.createDriver();
      return driver.findElements(By.tagName('a')).then(function (elements) {
    ...
    it('byJs_filtersOutNonWebElementResponses', function () {
      var ids = ['foo', 'bar', 'baz'];
      var json = [WebElement.buildId(ids[0]), 123, 'a', false, WebElement.buildId(ids[1]), { 'not a web element': 1 }, WebElement.buildId(ids[2])];
      let executor = new FakeExecutor().expect(CName.EXECUTE_SCRIPT, {
        'script': 'return document.getElementsByTagName("div");',
        'args': []
      }).andReturnSuccess(json).end();
    ...
    it('byJs_canPassScriptArguments', function () {
      ...
      let executor = new FakeExecutor().expect(CName.EXECUTE_SCRIPT, {
        'script': script,
        'args': ['div']
      }).andReturnSuccess([WebElement.buildId('one'), WebElement.buildId('two')]).end();
  ...
  describe('sendKeys', function () {
    it('convertsVarArgsIntoStrings_simpleArgs', function () {
      let executor = new FakeExecutor().expect(CName.SEND_KEYS_TO_ELEMENT, { 'id': WebElement.buildId('one'),
        'value': '12abc3'.split('') }).andReturnSuccess().end();
    ...
    it('convertsVarArgsIntoStrings_promisedArgs', function () {
      let executor = new FakeExecutor().expect(CName.FIND_ELEMENT, { 'using': 'css selector', 'value': '*[id="foo"]' }).andReturnSuccess(WebElement.buildId('one')).expect(CName.SEND_KEYS_TO_ELEMENT, { 'id': WebElement.buildId('one'),
        'value': 'abc123def'.split('') }).andReturnSuccess().end();
    ...
    it('sendKeysWithAFileDetector', function () {
      let executor = new FakeExecutor().expect(CName.FIND_ELEMENT, { 'using': 'css selector', 'value': '*[id="foo"]' }).andReturnSuccess(WebElement.buildId('one')).expect(CName.SEND_KEYS_TO_ELEMENT, { 'id': WebElement.buildId('one'),
        'value': 'modified/path'.split('') }).andReturnSuccess().end();
  ...
  describe("switchTo()", function () {
    describe("window", function () {
      ...
      it('should propagate exceptions', function () {
        ...
        return executor.createDriver().switchTo().window('foo').then(assert.fail, v => assert.strictEqual(v, e));
      });
    });
  });
  ...
  describe('waiting', function () {
    describe('supports custom wait functions', function () {
      it('waitSucceeds', function () {
        let executor = new FakeExecutor().expect(CName.FIND_ELEMENTS, { using: 'css selector', value: '*[id="foo"]' }).andReturnSuccess([]).times(2).expect(CName.FIND_ELEMENTS, { using: 'css selector', value: '*[id="foo"]' }).andReturnSuccess([WebElement.buildId('bar')]).end();
      ...
      it('waitTimesout_timeoutCaught', function () {
        let executor = new FakeExecutor().expect(CName.FIND_ELEMENTS, { using: 'css selector', value: '*[id="foo"]' }).andReturnSuccess([]).anyTimes().end();

        var driver = executor.createDriver();
        return driver.wait(function () {
          return driver.findElements(By.id('foo')).then(els => els.length > 0);
        }, 25).then(fail, function (e) {
          assert.equal('Wait timed out after ', e.message.substring(0, 'Wait timed out after '.length));
        });
      });
    ...
    describe('supports condition objects', function () {
      ...
      it('wait times out', function () {
        ...
        let driver = executor.createDriver();
        return driver.wait(until.elementLocated(By.id('foo')), 5).then(fail, err => assert.ok(err instanceof error.TimeoutError));
      });
    });
  ...
  describe('alert handling', function () {
    ...
    it('cannotSwitchToAlertThatIsNotPresent', function () {
      ...
      return executor.createDriver().switchTo().alert().then(assert.fail, v => assert.strictEqual(v, e));
    });

    enablePromiseManager(() => {
      it('alertsBelongToSameFlowAsParentDriver', function () {
        let executor = new FakeExecutor().expect(CName.GET_ALERT_TEXT).andReturnSuccess('hello').end();

        var driver = executor.createDriver();
        var otherFlow = new promise.ControlFlow();
        otherFlow.execute(function () {
          driver.switchTo().alert().then(function () {
            assert.strictEqual(driver.controlFlow(), promise.controlFlow(), 'Alert should belong to the same flow as its parent driver');
          });
        });

        assert.notEqual(otherFlow, driver.controlFlow);
        return Promise.all([waitForIdle(otherFlow), waitForIdle(driver.controlFlow())]);
      });
    });
    ...
    it('commandsFailIfAlertNotPresent', function () {
      ...
      return alert.getText().then(fail, expectedError).then(() => alert.accept()).then(fail, expectedError).then(() => alert.dismiss()).then(fail, expectError).then(() => alert.sendKeys('hi')).then(fail, expectError);
    });
  });

  enablePromiseManager(() => {
    it('testWebElementsBelongToSameFlowAsParentDriver', function () {
      let executor = new FakeExecutor().expect(CName.FIND_ELEMENT, { using: 'css selector', value: '*[id="foo"]' }).andReturnSuccess(WebElement.buildId('abc123')).end();
  ...
  it('testFetchingLogs', function () {
    let executor = new FakeExecutor().expect(CName.GET_LOG, { 'type': 'browser' }).andReturnSuccess([{ 'level': 'INFO', 'message': 'hello', 'timestamp': 1234 }, { 'level': 'DEBUG', 'message': 'abc123', 'timestamp': 5678 }]).end();
  ...
  it('testWebElementCommansFailIfElementCouldNotBeFound', function () {
    ...
    let executor = new FakeExecutor().expect(CName.FIND_ELEMENT, { using: 'css selector', value: '*[id="foo"]' }).andReturnError(e).end();

    var driver = executor.createDriver();
    return driver.findElement(By.id('foo')).click().then(fail, v => assert.strictEqual(v, e));
  });
  ...
  it('testCannotFindChildElementsIfParentCouldNotBeFound', function () {
    ...
    var driver = executor.createDriver();
    return driver.findElement(By.id('foo')).findElement(By.id('bar')).findElement(By.id('baz')).then(fail, v => assert.strictEqual(v, e));
  });
  ...
  describe('actions()', function () {
    it('failsIfInitialDriverCreationFailed', function () {
      ...
      return driver.actions().mouseDown().mouseUp().perform().catch(assertIsStubError);
    });

    describe('mouseMove', function () {
      it('noElement', function () {
        let executor = new FakeExecutor().expect(CName.MOVE_TO, { 'xoffset': 0, 'yoffset': 125 }).andReturnSuccess().end();

        return executor.createDriver().actions().mouseMove({ x: 0, y: 125 }).perform();
      });

      it('element', function () {
        let executor = new FakeExecutor().expect(CName.FIND_ELEMENT, { using: 'css selector', value: '*[id="foo"]' }).andReturnSuccess(WebElement.buildId('abc123')).expect(CName.MOVE_TO, { 'element': 'abc123', 'xoffset': 0, 'yoffset': 125 }).andReturnSuccess().end();

        var driver = executor.createDriver();
        var element = driver.findElement(By.id('foo'));
        return driver.actions().mouseMove(element, { x: 0, y: 125 }).perform();
      });
    });

    it('supportsMouseDown', function () {
      let executor = new FakeExecutor().expect(CName.MOUSE_DOWN, { 'button': Button.LEFT }).andReturnSuccess().end();

      return executor.createDriver().actions().mouseDown().perform();
    });

    it('testActionSequence', function () {
      let executor = new FakeExecutor().expect(CName.FIND_ELEMENT, { using: 'css selector', value: '*[id="a"]' }).andReturnSuccess(WebElement.buildId('id1')).expect(CName.FIND_ELEMENT, { using: 'css selector', value: '*[id="b"]' }).andReturnSuccess(WebElement.buildId('id2')).expect(CName.SEND_KEYS_TO_ACTIVE_ELEMENT, { 'value': [Key.SHIFT] }).andReturnSuccess().expect(CName.MOVE_TO, { 'element': 'id1' }).andReturnSuccess().expect(CName.CLICK, { 'button': Button.LEFT }).andReturnSuccess().expect(CName.MOVE_TO, { 'element': 'id2' }).andReturnSuccess().expect(CName.CLICK, { 'button': Button.LEFT }).andReturnSuccess().end();

      var driver = executor.createDriver();
      var element1 = driver.findElement(By.id('a'));
      var element2 = driver.findElement(By.id('b'));

      return driver.actions().keyDown(Key.SHIFT).click(element1).click(element2).perform();
    });
  });
  ...
  describe('touchActions()', function () {
    it('failsIfInitialDriverCreationFailed', function () {
      ...
      let driver = new FakeExecutor().createDriver(session);
      driver.getSession().catch(function () {});
      return driver.touchActions().scroll({ x: 3, y: 4 }).perform().catch(assertIsStubError);
    });

    it('testTouchActionSequence', function () {
      let executor = new FakeExecutor().expect(CName.TOUCH_DOWN, { x: 1, y: 2 }).andReturnSuccess().expect(CName.TOUCH_MOVE, { x: 3, y: 4 }).andReturnSuccess().expect(CName.TOUCH_UP, { x: 5, y: 6 }).andReturnSuccess().end();

      var driver = executor.createDriver();
      return driver.touchActions().tapAndHold({ x: 1, y: 2 }).move({ x: 3, y: 4 }).release({ x: 5, y: 6 }).perform();
    });
  });
  ...
  describe('manage()', function () {
    describe('setTimeouts()', function () {
      ...
      it('can set multiple timeouts', function () {
        let executor = new FakeExecutor().expect(CName.SET_TIMEOUT, { script: 1, pageLoad: 2, implicit: 3 }).andReturnSuccess().end();
        let driver = executor.createDriver();
        return driver.manage().setTimeouts({ script: 1, pageLoad: 2, implicit: 3 });
      });

      it('falls back to legacy wire format if W3C version fails', () => {
        let executor = new FakeExecutor().expect(CName.SET_TIMEOUT, { implicit: 3 }).andReturnError(Error('oops')).expect(CName.SET_TIMEOUT, { type: 'implicit', ms: 3 }).andReturnSuccess().end();
        let driver = executor.createDriver();
      ...
      describe('deprecated API calls setTimeouts()', function () {
        it('implicitlyWait()', function () {
          let executor = new FakeExecutor().expect(CName.SET_TIMEOUT, { implicit: 3 }).andReturnSuccess().end();
          let driver = executor.createDriver();
          return driver.manage().timeouts().implicitlyWait(3);
        });

        it('setScriptTimeout()', function () {
          let executor = new FakeExecutor().expect(CName.SET_TIMEOUT, { script: 3 }).andReturnSuccess().end();
          let driver = executor.createDriver();
          return driver.manage().timeouts().setScriptTimeout(3);
        });

        it('pageLoadTimeout()', function () {
          let executor = new FakeExecutor().expect(CName.SET_TIMEOUT, { pageLoad: 3 }).andReturnSuccess().end();
          let driver = executor.createDriver();
  ...
  describe('generator support', function () {
    ...
    beforeEach(function () {
      driver = new WebDriver(new Session('test-session', {}), new ExplodingExecutor());
    });
  ...
  describe('wire format', function () {
    ...
    describe('can serialize', function () {
      function runSerializeTest(input, want) {
        ...
        return WebDriver.createSession(executor, input).getSession();
      }
      ...
      it('object with toJSON()', function () {
        return runSerializeTest(new Date(605728511546), '1989-03-12T17:55:11.546Z');
      });
      ...
      it('WebElement', function () {
        return runSerializeTest(new WebElement(FAKE_DRIVER, 'fefifofum'), WebElement.buildId('fefifofum'));
      });

      it('WebElementPromise', function () {
        return runSerializeTest(new WebElementPromise(FAKE_DRIVER, Promise.resolve(new WebElement(FAKE_DRIVER, 'fefifofum'))), WebElement.buildId('fefifofum'));
      });
      ...
      describe('an array', function () {
        ...
        it('complex array', function () {
          var expected = ['abc', 123, true, WebElement.buildId('fefifofum'), [123, { 'foo': 'bar' }]];
        ...
        it('nested promises', function () {
          return runSerializeTest(['abc', Promise.resolve([123, Promise.resolve(true)])], ['abc', [123, true]]);
        });
      });
      ...
      describe('an object', function () {
        ...
        it('with values that have toJSON', function () {
          return runSerializeTest({ a: { b: new Date(605728511546) } }, { a: { b: '1989-03-12T17:55:11.546Z' } });
        });
    ...
    describe('can deserialize', function () {
      function runDeserializeTest(original, want) {
        let executor = new FakeExecutor().expect(CName.GET_CURRENT_URL).andReturnSuccess(original).end();
        let driver = executor.createDriver();
      ...
      it('primitives', function () {
        return Promise.all([runDeserializeTest(1, 1), runDeserializeTest('', ''), runDeserializeTest(true, true), runDeserializeTest(undefined, undefined), runDeserializeTest(null, null)]);
      });
      ...
      it('passes through function properties', function () {
        function bar() {}
        return runDeserializeTest([{ foo: { 'bar': 123 }, func: bar }], [{ foo: { 'bar': 123 }, func: bar }]);
      });
    });
  });
});

//# sourceMappingURL=webdriver_test-compiled.js.map