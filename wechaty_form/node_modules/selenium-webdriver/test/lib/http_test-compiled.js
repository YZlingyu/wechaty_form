...
describe('http', function () {
  describe('buildPath', function () {
    ...
    it('handles web element references', function () {
      ...
      var finalPath = http.buildPath('/session/:sessionId/element/:id/click', parameters);
      assert.equal(finalPath, '/session/foo/element/bar/click');
      assert.deepEqual(parameters, {});
    });

    it('throws if missing a parameter', function () {
      assert.throws(() => http.buildPath('/session/:sessionId', {}), function (err) {
        return err instanceof error.InvalidArgumentError && 'Missing required parameter: sessionId' === err.message;
      });

      assert.throws(() => http.buildPath('/session/:sessionId/element/:id', { 'sessionId': 'foo' }), function (err) {
    ...
    it('does not match on segments that do not start with a colon', function () {
      assert.equal(http.buildPath('/session/foo:bar/baz', {}), '/session/foo:bar/baz');
    });
  });
  ...
  describe('Executor', function () {
    ...
    describe('command routing', function () {
      ...
      it('rejects commands missing URL parameters', function () {
        let command = new Command(CommandName.FIND_CHILD_ELEMENT).setParameter('sessionId', 's123').
        // Let this be missing: setParameter('id', {'ELEMENT': 'e456'}).
        setParameter('using', 'id').setParameter('value', 'foo');

        assert.throws(() => executor.execute(command), function (err) {
          return err instanceof error.InvalidArgumentError && 'Missing required parameter: id' === err.message;
        });
      ...
      it('replaces URL parameters with command parameters', function () {
        ...
        return assertSendsSuccessfully(command).then(function (response) {
          assertSent('POST', '/session/s123/url', { 'url': 'http://www.google.com' }, [['Accept', 'application/json; charset=utf-8']]);
        });
      });
      ...
      describe('uses correct URL', function () {
        ...
        describe('in legacy mode', function () {
          test(CommandName.GET_WINDOW_SIZE, { sessionId: 's123' }, false, 'GET', '/session/s123/window/current/size');

          test(CommandName.SET_WINDOW_SIZE, { sessionId: 's123', width: 1, height: 1 }, false, 'POST', '/session/s123/window/current/size', { width: 1, height: 1 });

          test(CommandName.MAXIMIZE_WINDOW, { sessionId: 's123' }, false, 'POST', '/session/s123/window/current/maximize');

          // This is consistent b/w legacy and W3C, just making sure.
          test(CommandName.GET, { sessionId: 's123', url: 'http://www.example.com' }, false, 'POST', '/session/s123/url', { url: 'http://www.example.com' });
        });

        describe('in W3C mode', function () {
          test(CommandName.GET_WINDOW_SIZE, { sessionId: 's123' }, true, 'GET', '/session/s123/window/size');

          test(CommandName.SET_WINDOW_SIZE, { sessionId: 's123', width: 1, height: 1 }, true, 'POST', '/session/s123/window/size', { width: 1, height: 1 });

          test(CommandName.MAXIMIZE_WINDOW, { sessionId: 's123' }, true, 'POST', '/session/s123/window/maximize');

          // This is consistent b/w legacy and W3C, just making sure.
          test(CommandName.GET, { sessionId: 's123', url: 'http://www.example.com' }, true, 'POST', '/session/s123/url', { url: 'http://www.example.com' });
        });
        ...
        function test(command, parameters, w3c, expectedMethod, expectedUrl, opt_expectedParams) {
          it(`command=${ command }`, function () {
            ...
            return executor.execute(cmd).then(function () {
              assertSent(expectedMethod, expectedUrl, opt_expectedParams || {}, [['Accept', 'application/json; charset=utf-8']]);
            });
          });
        }
      });
    ...
    describe('response parsing', function () {
      ...
      describe('extracts Session from NEW_SESSION response', function () {
          ...
          describe('(empty response)', function () {
            ...
            function test(w3c) {
              it('w3c === ' + w3c, function () {
                send.returns(Promise.resolve(new http.Response(200, {}, '')));
                executor.w3c = w3c;
                return executor.execute(command).then(() => assert.fail('expected to fail'), e => {
        ...
        it('handles legacy new session failures', function () {
          ...
          return executor.execute(command).then(() => assert.fail('should have failed'), e => {
        ...
        it('handles w3c new session failures', function () {
          let rawResponse = { value: { error: 'no such element', message: 'oops' } };
      ...
      describe('extracts Session from DESCRIBE_SESSION response', function () {
        ...
        beforeEach(function () {
          executor = new http.Executor(client);
          command = new Command(CommandName.DESCRIBE_SESSION).setParameter('sessionId', 'foo');
        });
      ...
      it('throws UnsupportedOperationError for 404 and body not JSON', function () {
        ...
        return executor.execute(command).then(() => assert.fail('should have failed'), checkError(error.UnsupportedOperationError, 'hello, world\ngoodbye, world!'));
      });
      ...
      it('throws WebDriverError for generic 4xx when body not JSON', function () {
        ...
        return executor.execute(command).then(() => assert.fail('should have failed'), checkError(error.WebDriverError, 'hello, world\ngoodbye, world!')).then(function () {
          assertSent('GET', '/session/s123/url', {}, [['Accept', 'application/json; charset=utf-8']]);
        });
      });
    });
...
//# sourceMappingURL=http_test-compiled.js.map