...
test.suite(function (env) {
  ...
  test.it('should follow redirects sent in the http response headers', function* () {
    yield driver.get(Pages.redirectPage);
    return assert(driver.getTitle()).equalTo('We Arrive Here');
  });

  test.ignore(browsers(Browser.SAFARI)).it('should follow meta redirects', function* () {
    yield driver.get(Pages.metaRedirectPage);
    return assert(driver.getTitle()).equalTo('We Arrive Here');
  });

  // Skip Firefox; see https://bugzilla.mozilla.org/show_bug.cgi?id=1280300
  test.ignore(browsers(Browser.FIREFOX)).it('should be able to get a fragment on the current page', function* () {
  ...
  describe('timeouts', function () {
    ...
    test.ignore(browsers(Browser.CHROME, Browser.IE, Browser.IPAD, Browser.IPHONE, Browser.OPERA, Browser.PHANTOM_JS)).it('should timeout if page load timeout is set', function* () {
      yield driver.manage().timeouts().pageLoadTimeout(1);
      return driver.get(Pages.sleepingPage + '?time=3').then(function () {
...
//# sourceMappingURL=page_loading_test-compiled.js.map