...
test.suite(function (env) {
  ...
  function writePacFile(res) {
    writeResponse(res, ['function FindProxyForURL(url, host) {', '  if (shExpMatch(url, "' + goodbyeServer.url('*') + '")) {', '    return "DIRECT";', '  }', '  return "PROXY ' + proxyServer.host() + '";', '}'].join('\n'), 'ascii', 'application/x-javascript-config');
  }
  ...
  var proxyServer = new Server(function (req, res) {
    ...
    writeResponse(res, ['<!DOCTYPE html>', '<title>Proxy page</title>', '<h3>This is the proxy landing page</h3>'].join(''), 'utf8', 'text/html; charset=UTF-8');
  });

  var helloServer = new Server(function (req, res) {
    writeResponse(res, ['<!DOCTYPE html>', '<title>Hello</title>', '<h3>Hello, world!</h3>'].join(''), 'utf8', 'text/html; charset=UTF-8');
  });

  var goodbyeServer = new Server(function (req, res) {
    writeResponse(res, ['<!DOCTYPE html>', '<title>Goodbye</title>', '<h3>Goodbye, world!</h3>'].join(''), 'utf8', 'text/html; charset=UTF-8');
  });
  ...
  function createDriver(proxy) {
    ...
    return driver = env.builder().setFirefoxOptions(new firefox.Options().setProfile(profile)).setProxy(proxy).build();
  }
  ...
  test.ignore(env.browsers(Browser.IE, Browser.OPERA, Browser.SAFARI)).describe('manual proxy settings', function () {
    ...
    test.ignore(env.browsers(Browser.PHANTOM_JS)).it('can configure HTTP proxy host', function* () {
      ...
      yield assert(driver.findElement({ tagName: 'h3' }).getText()).equalTo('This is the proxy landing page');
    });
    ...
    test.ignore(env.browsers(Browser.FIREFOX, 'legacy-' + Browser.FIREFOX, Browser.PHANTOM_JS)).it('can bypass proxy for specific hosts', function* () {
      yield createDriver(proxy.manual({
        http: proxyServer.host(),
      ...
      yield assert(driver.findElement({ tagName: 'h3' }).getText()).equalTo('Hello, world!');
  ...
  test.ignore(env.browsers(Browser.IE, Browser.OPERA, Browser.PHANTOM_JS, Browser.SAFARI)).describe('pac proxy settings', function () {
    test.it('can configure proxy through PAC file', function* () {
      ...
      yield assert(driver.findElement({ tagName: 'h3' }).getText()).equalTo('Goodbye, world!');
    });
  });
