...
var JETPACK_EXTENSION = path.join(__dirname, '../../lib/test/data/firefox/jetpack-sample.xpi');
var NORMAL_EXTENSION = path.join(__dirname, '../../lib/test/data/firefox/sample.xpi');
...
test.suite(function (env) {
  describe('firefox', function () {
    describe('Options', function () {
      ...
      function runWithFirefoxDev(options, testFn) {
        return firefox.Channel.AURORA.locate().then(exe => {
          ...
          console.warn('Skipping test: could not find Firefox Dev Edition: ' + err);
        });
      }
      ...
      describe('can start Firefox with custom preferences', function () {
        function runTest(opt_dir) {
          return consume(function* () {
            ...
            var userAgent = yield driver.executeScript('return window.navigator.userAgent');
      ...
      test.it('can start Firefox with a jetpack extension', function () {
        ...
        return runWithFirefoxDev(options, function* () {
          yield loadJetpackPage(driver, 'data:text/html;charset=UTF-8,<html><div>content</div></html>');
      ...
      test.it('can start Firefox with a normal extension', function () {
        ...
        return runWithFirefoxDev(options, function* () {
          ...
          let footer = yield driver.findElement({ id: 'sample-extension-footer' });
      ...
      test.it('can start Firefox with multiple extensions', function () {
        ...
        return runWithFirefoxDev(options, function* () {
          ...
          let footer = yield driver.findElement({ id: 'sample-extension-footer' }).getText();
      ...
      function loadJetpackPage(driver, url) {
        ...
        return driver.wait(function () {
          driver.get(url);
          return driver.findElements({ id: 'jetpack-sample-banner' }).then(found => found.length > 0);
        }, 3000);
      }
    });
    ...
    describe('context switching', function () {
      ...
      test.ignore(() => !env.isMarionette).it('can get context', function () {
        return assert(driver.getContext()).equalTo(Context.CONTENT);
      });

      test.ignore(() => !env.isMarionette).it('can set context', function* () {
      ...
      test.ignore(() => !env.isMarionette).it('throws on unknown context', function () {
...
//# sourceMappingURL=firefox_test-compiled.js.map