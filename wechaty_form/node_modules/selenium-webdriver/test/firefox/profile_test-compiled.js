...
describe('Profile', function () {
  describe('setPreference', function () {
    ...
    it('allows overriding mutable properties', function () {
      ...
      assert.equal('http://www.example.com', profile.getPreference('browser.newtab.url'));
    });
  ...
  describe('writeToDisk', function () {
    it('copies template directory recursively', function () {
      ...
      return io.tmpDir().then(function (td) {
        ...
        assert.equal('Hello, world', fs.readFileSync(path.join(profileDir, 'foo')));
        assert.equal('Goodbye, world!', fs.readFileSync(path.join(profileDir, 'subfolder/bar')));
      });
    });
    ...
    describe('user.js', function () {
      ...
      it('merges template user.js into preferences', function () {
        return io.tmpDir().then(function (dir) {
          fs.writeFileSync(path.join(dir, 'user.js'), ['user_pref("browser.newtab.url", "http://www.example.com")', 'user_pref("dom.max_script_run_time", 1234)'].join('\n'));
      ...
      it('ignores frozen preferences when merging template user.js', function () {
        return io.tmpDir().then(function (dir) {
          fs.writeFileSync(path.join(dir, 'user.js'), 'user_pref("app.update.auto", true)');
          return new Profile(dir).writeToDisk();
        }).then(function (profile) {
          return loadUserPrefs(path.join(profile, 'user.js'));
        }).then(function (prefs) {
...
//# sourceMappingURL=profile_test-compiled.js.map