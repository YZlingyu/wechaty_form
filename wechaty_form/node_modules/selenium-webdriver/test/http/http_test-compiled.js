...
describe('HttpClient', function () {
  ...
  it('can send a basic HTTP request', function () {
    ...
    return client.send(request).then(function (response) {
      ...
      assert.equal(request.headers.get('Accept'), 'application/json; charset=utf-8');
    });
  });
  ...
  describe('with proxy', function () {
    it('sends request to proxy with absolute URI', function () {
      ...
      return client.send(request).then(function (response) {
        ...
        assert.equal(response.headers.get('x-proxy-request-uri'), 'http://another.server.com/proxy');
      });
    });
    ...
    it('includes search and hash in redirect URI', function () {
      ...
      return client.send(request).then(function (response) {
        assert.equal(200, response.status);
        assert.equal(response.headers.get('host'), 'another.server.com');
        assert.equal(response.headers.get('x-proxy-request-uri'), 'http://another.server.com/proxy?foo#bar');
      });
    });
  });
});
