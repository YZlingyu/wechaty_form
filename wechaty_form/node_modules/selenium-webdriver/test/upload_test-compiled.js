...
test.suite(function (env) {
  ...
  test.ignore(env.browsers(Browser.IPAD, Browser.IPHONE,
  // Uploads broken in PhantomJS 2.0.
  // See https://github.com/ariya/phantomjs/issues/12506
  Browser.PHANTOM_JS, Browser.SAFARI)).it('can upload files', function* () {
    ...
    yield driver.wait(until.elementIsNotVisible(label), 10 * 1000, 'File took longer than 10 seconds to upload!');

    var frame = yield driver.findElement(By.id('upload_target'));
    yield driver.switchTo().frame(frame);
    yield assert(driver.findElement(By.css('body')).getText()).equalTo(LOREM_IPSUM_TEXT);
  });
});

//# sourceMappingURL=upload_test-compiled.js.map