...
describe('DriverService', function () {
  describe('start()', function () {
    ...
    enablePromiseManager(function () {
      describe('failures propagate through control flow if child-process dies', function () {
        it('', function () {
          this.timeout(1000);
...
describe('FileDetector', function () {
  ...
  it('returns the original path if the file does not exist', function () {
    return io.tmpDir(dir => {
      ...
      return new remote.FileDetector().handleFile(new ExplodingDriver(), theFile).then(f => assert.equal(f, theFile));
    });
  });

  it('returns the original path if it is a directory', function () {
    return io.tmpDir(dir => {
      return new remote.FileDetector().handleFile(new ExplodingDriver(), dir).then(f => assert.equal(f, dir));
    });
  });

  it('attempts to upload valid files', function () {
    return io.tmpFile(theFile => {
      return new remote.FileDetector().handleFile(new class FakeDriver {
        schedule(command) {
          assert.equal(command.getName(), cmd.Name.UPLOAD_FILE);
          assert.equal(typeof command.getParameters()['file'], 'string');
          return Promise.resolve('success!');
        }
      }(), theFile).then(f => assert.equal(f, 'success!'));
    });
  });
});

//# sourceMappingURL=remote_test-compiled.js.map