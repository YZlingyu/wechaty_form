...
/** @const */
const WEBDRIVER_PREFERENCES_PATH = isDevMode ? path.join(__dirname, '../../../firefox-driver/webdriver.json') : path.join(__dirname, '../lib/firefox/webdriver.json');

/** @const */
const WEBDRIVER_EXTENSION_PATH = isDevMode ? path.join(__dirname, '../../../../build/javascript/firefox-driver/webdriver.xpi') : path.join(__dirname, '../lib/firefox/webdriver.xpi');
...
function getDefaultPreferences() {
  if (!defaultPreferences) {
    var contents = /** @type {string} */fs.readFileSync(WEBDRIVER_PREFERENCES_PATH, 'utf8');
...
function decode(data) {
  return io.tmpFile().then(function (file) {
    ...
    return io.write(file, buf).then(io.tmpDir).then(function (dir) {
  ...
  writeToDisk(opt_excludeWebDriverExt) {
    ...
    if (this.template_) {
      profileDir = profileDir.then(function (dir) {
        return io.copyDir(
        /** @type {string} */this.template_, dir, /(parent\.lock|lock|\.parentlock)/);
      }.bind(this));
    }
