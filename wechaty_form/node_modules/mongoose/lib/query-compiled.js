...
function Query(conditions, options, model, collection) {
  ...
  if (this.schema) {
    ...
    this._count = this.model.hooks.createWrapper('count', Query.prototype._count, this, kareemOptions);
    this._execUpdate = this.model.hooks.createWrapper('update', Query.prototype._execUpdate, this, kareemOptions);
    this._find = this.model.hooks.createWrapper('find', Query.prototype._find, this, kareemOptions);
    this._findOne = this.model.hooks.createWrapper('findOne', Query.prototype._findOne, this, kareemOptions);
    this._findOneAndRemove = this.model.hooks.createWrapper('findOneAndRemove', Query.prototype._findOneAndRemove, this, kareemOptions);
    this._findOneAndUpdate = this.model.hooks.createWrapper('findOneAndUpdate', Query.prototype._findOneAndUpdate, this, kareemOptions);
    this._replaceOne = this.model.hooks.createWrapper('replaceOne', Query.prototype._replaceOne, this, kareemOptions);
    this._updateMany = this.model.hooks.createWrapper('updateMany', Query.prototype._updateMany, this, kareemOptions);
    this._updateOne = this.model.hooks.createWrapper('updateOne', Query.prototype._updateOne, this, kareemOptions);
  }
}
...
Query.prototype._find = function (callback) {
  ...
  var cb = function (err, docs) {
    ...
    if (!options.populate) {
      return options.lean === true ? callback(null, docs) : completeMany(_this.model, docs, fields, null, callback);
    }
    ...
    _this.model.populate(docs, pop, function (err, docs) {
      ...
      return options.lean === true ? callback(null, docs) : completeMany(_this.model, docs, fields, pop, callback);
    });
  };
...
Query.prototype._findOne = function (callback) {
  ...
  Query.base.findOne.call(_this, {}, function (err, doc) {
    ...
    if (!options.populate) {
      return options.lean === true ? callback(null, doc) : completeOne(_this.model, doc, null, {}, projection, null, callback);
    }
    ...
    _this.model.populate(doc, pop, function (err, doc) {
      ...
      return options.lean === true ? callback(null, doc) : completeOne(_this.model, doc, null, {}, projection, pop, callback);
    });
  });
};
...
Query.prototype.findOne = function (conditions, projection, options, callback) {
  ...
  if (mquery.canMerge(conditions)) {
    ...
    throw new Error('Invalid argument to findOne(): ' + util.inspect(conditions));
  }
...
function _update(query, op, conditions, doc, options, callback) {
  ...
  try {
    ...
    castedDoc = query._castUpdate(utils.clone(doc, $options), options && options.overwrite || op === 'replaceOne');
  } catch (err) {
    query._castError = castedQuery;
    if (callback) {
      callback(err);
...
Query.prototype.cast = function (model, obj) {
  ...
  try {
    return cast(model.schema, obj, {
      ...
      strict: this.options && this.options.strict || model.schema.options && model.schema.options.strict
    });
  } catch (err) {
...
Query.prototype.stream = util.deprecate(Query.prototype.stream, 'Mongoose: ' + 'Query.prototype.stream() is deprecated in mongoose >= 4.5.0, ' + 'use Query.prototype.cursor() instead');
