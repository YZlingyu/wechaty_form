...
promise = promise.then(function () {
  if (chromedriver_version === 'LATEST') return getLatestVersion(getRequestOptions(cdnUrl + '/LATEST_RELEASE'));
});
...
function findSuitableTempDirectory() {
  var now = Date.now();
  var candidateTmpDirs = [process.env.TMPDIR || process.env.TMP || process.env.npm_config_tmp, '/tmp', path.join(process.cwd(), 'tmp')];
...
function getRequestOptions(downloadPath) {
  ...
  var proxyUrl = options.protocol === 'https:' ? process.env.npm_config_https_proxy : process.env.npm_config_proxy || process.env.npm_config_http_proxy;
  if (proxyUrl) {
  ...
  if (!ca && process.env.npm_config_cafile) {
    try {
      ca = fs.readFileSync(process.env.npm_config_cafile, { encoding: 'utf8' }).split(/\n(?=-----BEGIN CERTIFICATE-----)/g);
