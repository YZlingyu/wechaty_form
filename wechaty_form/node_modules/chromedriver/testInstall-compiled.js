...
function removeFolder(dir) {
    ...
    fs.readdirSync(dir).forEach(file => {
        ...
        if (fs.lstatSync(curPath).isDirectory()) removeFolder(curPath);else fs.unlinkSync(curPath);
    });
    fs.rmdirSync(dir);
}
...
function checkSpawn(spawnInfo) {
    if (spawnInfo.stdout) {
        if (typeof spawnInfo.stdout !== 'string') console.log(spawnInfo.stdout.toString('utf8'));else console.log(spawnInfo.stdout);
    }
    if (spawnInfo.stderr) {
        if (typeof spawnInfo.error !== 'string') console.error(spawnInfo.stderr.toString('utf8'));else console.error(spawnInfo.stderr);
    }
    if (spawnInfo.status !== 0 || spawnInfo.error) {
        console.error('Failed when spawning.');
        process.exit(1);
    }
    if (typeof spawnInfo.stdout !== 'string') return spawnInfo.stdout.toString('utf8');else return spawnInfo.stdout;
}

function nvmUse(version) {
    if (os.platform() === 'win32') var versionsText = checkSpawn(spawnSync('nvm', ['list']));else var versionsText = checkSpawn(spawnSync('/bin/bash', ['-c', 'source $HOME/.nvm/nvm.sh && nvm list']));
    ...
    if (os.platform() === 'win32') checkSpawn(spawnSync('nvm', ['use', largestMatchingVersion]));else checkSpawn(spawnSync('/bin/bash', ['-c', `source $HOME/.nvm/nvm.sh && nvm use ${ largestMatchingVersion }`]));
}
...
//# sourceMappingURL=testInstall-compiled.js.map